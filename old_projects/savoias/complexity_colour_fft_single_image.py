...
#read path to image (maybe an argument for single image/image folder)
#grayscale or coloured
#calculate complexity+partial complexities
#draw debug image (in the same folder? special folder!)
#print full complexity and formatted list of partial complexities
...
try:
#complexity_colour_fft.py image 10.jpg
	analysis_type=sys.argv[1]# image/folder
	images_path=sys.argv[2]
except: 
	print("please run this script with appropriate parameters: complexity_colour_fft_single_image.py image image_path\n")
#cg_type=sys.argv[4]
...
############load sorted files##########################    
    i=0
    list_dir=[int(file.split(".")[0]) for file in os.listdir(image_path)]
    list_dir.sort()
