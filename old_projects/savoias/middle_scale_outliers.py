...
if subset_name=='advertisement': 
	idx=[199]#mcdonalds logo was ranked as 0 by humans while being technically somewhat complex.  85 and 86 are just bugs!
	outliers=df.loc[idx]
	df=df.drop(idx)

'''
if subset_name=='infographics':
	idx=[115, 99, 25, 117, 44, 68, 177, 140, 35]#these only have text. removing them improves performance but doesn't alter distribution shape FIXME no these are just bugs of coarse graining
	outliers=df.loc[idx]
	df=df.drop(idx)
'''

if subset_name=='art':
	idx=[225, 380, 119, 30, 286, 88]
	outliers=df.loc[idx]
	df=df.drop(idx)
'''
idx=[]
...
ttt=outliers['s'+str(fr2)]
for i in range(fr1,fr2):
	ttt=ttt+outliers['s'+str(i)]
outliers['frac']=ttt


ttt=i_outliers['s'+str(fr2)]
for i in range(fr1,fr2):
	ttt=ttt+i_outliers['s'+str(i)]
i_outliers['frac']=ttt
...
plt.scatter(xo,yo,color='red', linewidth=2, alpha=1)
plt.scatter(i_xo,i_yo,color='#f6984f', linewidth=2, alpha=1)
