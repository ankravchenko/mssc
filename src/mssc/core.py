...
#complexity_colour_fft.py suprematism "../Savoias-Dataset/Images/Suprematism/" "../Savoias-Dataset/Images/global_ranking/global_ranking_sup.xlsx" blur
	...
	print("computing MSSC for test.jpg by default. for a different file please run this script with appropriate parameters: mssc_base_script.py filename.jpg\n")
#cg_type=sys.argv[4]

print("read arguments")
############load sorted files##########################
#takes an image part as an argument #FIXME takes image, implement loading separately with all the catches
def calculate_mssc(fpath):
    img = Image.open(fpath)


    im=np.array(img)#(ImageOps.grayscale(im)))
    ...
    plt.plot(x, partial)
    plt.title("partial complexities")
    plt.xticks(x)
    plt.tight_layout()
    plt.savefig(fpath.split('.')[0]+"_partial_complexities.png")
    return x, partial
